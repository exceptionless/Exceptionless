(function () {
    ...
    .factory('stackService', ['filterService', 'Restangular', function (filterService, Restangular) {
        ...
        function getFrequentByProjectId(id, options) {
            return Restangular.one('projects', id).one('stacks').all('frequent').getList(filterService.apply(options));
        }

        function getNewByProjectId(id, options) {
            return Restangular.one('projects', id).one('stacks').all('new').getList(filterService.apply(options));
        }
