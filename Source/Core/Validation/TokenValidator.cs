...
namespace Exceptionless.Core.Validation {
    public class TokenValidator : AbstractValidator<Token> {
        public TokenValidator() {
            ...
            RuleFor(t => t.OrganizationId).NotEmpty().When(p => !String.IsNullOrEmpty(p.ProjectId) || !String.IsNullOrEmpty(p.DefaultProjectId)).WithMessage("Please specify a valid organization id.");
