    ...
    public class StripeEventHandler {
        ...
        public async Task HandleEventAsync(StripeEvent stripeEvent) {
            switch (stripeEvent.Type) {
                ...
                default: {
                        _logger.Trace().Message("Unhandled stripe webhook called. Type: {0} Id: {1} UserId: {2}", stripeEvent.Type, stripeEvent.Id, stripeEvent.UserId).Write();
                    break;
                }
            }
        }
        ...
        private async Task SubscriptionUpdatedAsync(StripeSubscription sub) {
            ...
            if (org == null) {
                _logger.Error().Message("Unknown customer id in updated subscription: {0}", sub.CustomerId).Write();
                return;
            }

            _logger.Info().Message("Stripe subscription updated. Customer: {0} Org: {1} Org Name: {2}", sub.CustomerId, org.Id, org.Name).Write();
        ...
        private async Task SubscriptionDeletedAsync(StripeSubscription sub) {
            var org = await _organizationRepository.GetByStripeCustomerIdAsync(sub.CustomerId).AnyContext();
            if (org == null) {
                _logger.Error().Message("Unknown customer id in deleted subscription: {0}", sub.CustomerId).Write();
                return;
            }

            _logger.Info().Message("Stripe subscription deleted. Customer: {0} Org: {1} Org Name: {2}", sub.CustomerId, org.Id, org.Name).Write();
        ...
        private async Task InvoicePaymentSucceededAsync(StripeInvoice inv) {
            ...
            if (org == null) {
                _logger.Error().Message("Unknown customer id in payment succeeded notification: {0}", inv.CustomerId).Write();
                return;
            }
            ...
            _logger.Info().Message("Stripe payment succeeded. Customer: {0} Org: {1} Org Name: {2}", inv.CustomerId, org.Id, org.Name).Write();
        }
