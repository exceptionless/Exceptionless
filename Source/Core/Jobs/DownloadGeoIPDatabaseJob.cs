        ...
        protected override async Task<JobResult> RunInternalAsync(JobRunContext context) {
            try {
                if (await _storage.ExistsAsync(MindMaxGeoIPResolver.GEO_IP_DATABASE_PATH).AnyContext()) {
                    ...
                    await _storage.DeleteFileAsync(MindMaxGeoIPResolver.GEO_IP_DATABASE_PATH, context.CancellationToken).AnyContext();
                }
                ...
                using (GZipStream decompressionStream = new GZipStream(await file.Content.ReadAsStreamAsync().AnyContext(), CompressionMode.Decompress))
                    await _storage.SaveFileAsync(MindMaxGeoIPResolver.GEO_IP_DATABASE_PATH, decompressionStream, context.CancellationToken).AnyContext();
            } catch (Exception ex) {
