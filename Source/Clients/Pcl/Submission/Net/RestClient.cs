...
namespace Exceptionless.Net {
    internal class RestClient {
        ...
        public RestClient(Uri baseUri, IJsonSerializer serializer) {
            BaseUri = baseUri;
        ...
        private void OnRequestStream(IAsyncResult ar) {
            ...
            try {
#if PORTABLE40
                using (Stream stream = state.Request.EndGetRequestStream(ar)) {
#else
                using (Stream requestStream = state.Request.EndGetRequestStream(ar)) {
                    using (var stream = new GZipStream(requestStream, CompressionMode.Compress)) {
#endif
                        if (state.RequestType == typeof(string))
                            WriteRequestString(stream, (string)state.RequestData);
                        else if (state.RequestType == typeof(byte[]))
                            WriteRequestBytes(stream, (byte[])state.RequestData);
                        else
                            WriteRequestType(stream, state.RequestData);
        ...
        private void WriteRequestType(Stream stream, object data) {
            using (var sw = new StreamWriter(stream))
                sw.Write(_serializer.Serialize(data));
        }
        ...
        private object ReadResponseType(Stream stream, RestState state) {
            ...
            return _serializer.Deserialize(json, state.ResponseType);
        }
