...
Set `EX_BaseURL` to the externally accessible Exceptionless URL and use an `EX_SmtpFrom` address authorized by your SMTP provider.
...
docker run --rm -it -p 5200:8080 -p 5089:443 \
    ...
    -e EX_BaseURL=https://example.com \
    -e 'EX_SmtpFrom=Exceptionless <noreply@example.com>' \
