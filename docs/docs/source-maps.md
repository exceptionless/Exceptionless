...
Exceptionless records a map's last successful use through a debounced cache update, then periodically persists that timestamp with the small metadata file instead of rewriting the map. The cleanup job removes unused maps after 14 days on free plans and 90 days on paid plans by default. Existing maps without usage metadata receive a full grace period on their first cleanup pass, and pending usage is checked before any map is removed. Stale maps are also cleaned before an upload or automatic download is rejected for exceeding storage limits.
