            ...
            function (
                ...
                function addHotKeys() {
                    ...
                    if (vm.event.stack_id) {
                        hotkeys.bindTo($scope).add({
                            ...
                            callback: function () {
                                $ExceptionlessClient
                                    .createFeatureUsage(vm._source + ".hotkeys.GoToStack")
                                    .addTags("hotkeys")
                        ...
                        if (clipboard.supported) {
                            hotkeys.bindTo($scope).add({
                                ...
                                callback: function () {
                                    $ExceptionlessClient
                                        .createFeatureUsage(vm._source + ".hotkeys.CopyEventJSON")
                                        .addTags("hotkeys")
                                        .setProperty("id", vm._eventId)
                                        .submit();
                    ...
                    if (vm.previous) {
                        hotkeys.bindTo($scope).add({
                            ...
                            callback: function () {
                                $ExceptionlessClient
                                    .createFeatureUsage(vm._source + ".hotkeys.PreviousOccurrence")
                                    .addTags("hotkeys")
                                    .setProperty("id", vm._eventId)
                                    .submit();
                    ...
                    if (vm.next) {
                        hotkeys.bindTo($scope).add({
                            ...
                            callback: function () {
                                $ExceptionlessClient
                                    .createFeatureUsage(vm._source + ".hotkeys.NextOccurrence")
                                    .addTags("hotkeys")
                                    .setProperty("id", vm._eventId)
                                    .submit();
                ...
                function buildTabs(tabNameToActivate) {
                    ...
                    if (vm.event.data && vm.event.data["@error"]) {
                        ...
                        tabs.push({
                            index: ++tabIndex,
                            title: translateService.T("Exception"),
                            template_key: "simple-error",
                        });
                    }
                    ...
                    if (vm.request && Object.keys(vm.request).length > 0) {
                        tabs.push({
                            index: ++tabIndex,
                            title: translateService.T(vm.isSessionStart ? "Browser" : "Request"),
                    ...
                    if (vm.environment && Object.keys(vm.environment).length > 0) {
                        tabs.push({
                            index: ++tabIndex,
                            title: translateService.T("Environment"),
                            template_key: "environment",
                        });
                    }
                    ...
                    if (extendedDataItems.length > 0) {
                        tabs.push({
                            index: ++tabIndex,
                            title: translateService.T("Extended Data"),
                            template_key: "extended-data",
                            data: extendedDataItems,
                        });
                    }
                    ...
                    if (vm.referenceId) {
                        tabs.push({
                            index: ++tabIndex,
                            title: translateService.T("Session Events"),
                ...
                function canRefresh(data) {
                    if (!!data && data.type === "PersistentEvent") {
                        ...
                        return filterService.includedInProjectOrOrganizationFilter({
                ...
                function demoteTab(tabName) {
                    function onSuccess() {
                        $ExceptionlessClient
                            .createFeatureUsage(vm._source + ".promoteTab.success")
                            .setProperty("id", vm._eventId)
                    ...
                    function onFailure(response) {
                        $ExceptionlessClient
                            .createFeatureUsage(vm._source + ".promoteTab.error")
                            .setProperty("id", vm._eventId)
                    ...
                    $ExceptionlessClient
                        .createFeatureUsage(vm._source + ".demoteTab")
                        .setProperty("id", vm._eventId)
                        .setProperty("TabName", tabName)
                        .submit();
                ...
                function getCurrentTab() {
                    var tab = vm.tabs.filter(function (t) {
                        return t.index === vm.activeTabIndex;
                    })[0];
                ...
                function getEvent() {
                    ...
                    function onSuccess(response) {
                        ...
                        vm.level =
                            vm.event.data && !!vm.event.data["@level"] ? vm.event.data["@level"].toLowerCase() : null;
                        ...
                        vm.hasCookies =
                            vm.request && !!vm.request.cookies && Object.keys(vm.request.cookies).length > 0;
                        vm.hasHeaders =
                            vm.request && !!vm.request.headers && Object.keys(vm.request.headers).length > 0;
                        vm.requestUrl =
                            vm.request &&
                            urlService.buildUrl(
                                vm.request.is_secure,
                                vm.request.host,
                                vm.request.port,
                                vm.request.path,
                    ...
                    return eventService
                        .getById(vm._eventId, {}, optionsCallback)
                ...
                function isPromoted(tabName) {
                    ...
                    return (
                        vm.project.promoted_tabs.filter(function (tab) {
                            return tab === tabName;
                        }).length > 0
                    );
                }
                ...
                function promoteTab(tabName) {
                    ...
                    $ExceptionlessClient
                        .createFeatureUsage(vm._source + ".promoteTab")
                        .setProperty("id", vm._eventId)
                        .setProperty("TabName", tabName)
                        .submit();
                ...
                function viewJSON() {
                    function onSuccess() {
                        $ExceptionlessClient
                            .createFeatureUsage(vm._source + ".viewJSON.success")
                            .setProperty("id", vm._eventId)
                            .submit();
                    }
                    ...
                    function onFailure() {
                        $ExceptionlessClient
                            .createFeatureUsage(vm._source + ".viewJSON.error")
                            .setProperty("id", vm._eventId)
                            .submit();
                    }

                    $ExceptionlessClient
                        .createFeatureUsage(vm._source + ".viewJSON")
                        .setProperty("id", vm._eventId)
                        .submit();
                    ...
                    vm.sessionEvents = {
                        ...
                        canRefresh: function canRefresh(events, data) {
                            if (data.type === "PersistentEvent") {
                                ...
                                return data.project_id
                                    ? vm.event.project_id === data.project_id
                                    : vm.event.organization_id === data.organization_id;
                            }
                        ...
                        get: function (options) {
                            function optionsCallback(options) {
                                ...
                                var start = vm.isSessionStart
                                    ? moment.utc(vm.event.date).local()
                                    : moment.utc(vm.event.date).subtract(180, "days").local();
                                var end =
                                    vm.event.data && vm.event.data.sessionend
                                        ? moment
                                              .utc(vm.event.data.sessionend)
                                              .add(1, "seconds")
                                              .local()
                            ...
                            return eventService.getBySessionId(
                                vm.event.project_id,
                                vm.referenceId,
                                options,
                                optionsCallback
