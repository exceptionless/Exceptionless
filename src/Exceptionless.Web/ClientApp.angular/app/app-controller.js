            ...
            function (
                ...
                function buildMenus() {
                    ...
                    function buildUrls() {
                        ...
                        result.reports.sessions = urlService.buildFilterUrl({
                            route: "events",
                            routePrefix: "session",
                            projectId: filterService.getProjectId(),
                            organizationId: filterService.getOrganizationId(),
                        ...
                        ["regressed", "fixed", "snoozed", "ignored", "discarded"].forEach(function (status) {
                            result.reports[status] = urlService.buildFilterUrl(
                                {
                                    moduleName: "app.reports",
                                    route: "status",
                                    projectId: filterService.getProjectId(),
                                    organizationId: filterService.getOrganizationId(),
                                },
                    ...
                    function isAllMenuActive(state, params) {
                        return (
                            dashboards.filter(function (dashboard) {
                                if (
                                    state.includes("app." + dashboard, params) ||
                    ...
                    function isSettingsMenuActive(state, params) {
                        return (
                            state.includes("app.project.list", params) ||
                    ...
                    function isTypeMenuActive(state, params, type) {
                        ...
                        return (
                            dashboards.filter(function (dashboard) {
                                if (
                                    state.includes("app.type-" + dashboard, parameters) ||
                    ...
                    var hasActiveMenu = Object.keys(isMenuActive).some(function (prop) {
                        return isMenuActive[prop];
                    });
                    if (hasActiveMenu) {
                ...
                function getUser(data) {
                    function onSuccess(response) {
                        ...
                        $ExceptionlessClient.config.setUserIdentity({
                            identity: vm.user.email_address,
