(function () {
    ...
    angular
        .module("exceptionless.event", ["restangular", "exceptionless.filter", "exceptionless.objectid"])
        .factory("eventService", function (filterService, objectIDService, organizationService, Restangular) {
            function calculateAveragePerHour(total, organizations) {
                ...
                range.start = moment.max(
                    [
                        range.start,
                        moment(filterService.getOldestPossibleEventDate()),
            ...
            function getBySessionId(projectId, id, options, optionsCallback) {
                ...
                return Restangular.one("projects", projectId)
                    .one("events", "sessions")
                    .all(id)
                    .getList(optionsCallback(filterService.apply(options, false)));
            }
