(function () {
    ...
    angular
        .module("exceptionless.state", ["angular-locker", "ui.router"])
        .factory("stateService", function ($location, $state, locker) {
            ...
            function restore(secondaryStateNameToRedirect, secondaryStateParams) {
                ...
                return secondaryStateNameToRedirect
                    ? $state.go(secondaryStateNameToRedirect, secondaryStateParams || {})
                    : $location.path("/");
            }
            ...
            function save(exclusions) {
                if (
                    exclusions &&
                    exclusions.filter(function (e) {
                        return $state.current.name.startsWith(e);
                    })[0]
                ) {
                    return;
                }
