(function () {
    ...
    angular.module("exceptionless.date-filter").directive("dateFilter", function ($interval, dateRangeParserService) {
        return {
            restrict: "E",
            replace: true,
            scope: true,
            ...
            controller: function ($interval, $scope, dialogs, filterService, translateService) {
                ...
                function getFilteredDisplayName() {
                    ...
                    if (range && range.start && range.end) {
                        return moment(range.start)
                            .twix(moment(range.end))
                            .format({ dayFormat: translateService.T("DayFormat") });
                    }
                ...
                function setCustomFilter() {
                    ...
                    return dialogs
                        .create(
                            "components/date-filter/custom-date-range-dialog.tpl.html",
                            "CustomDateRangeDialog as vm",
                            {
