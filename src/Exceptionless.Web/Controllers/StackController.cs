...
namespace Exceptionless.Web.Controllers {
    ...
    public class StackController : RepositoryApiController<IStackRepository, Stack, Stack, Stack, Stack> {
        ...
        public async Task<IActionResult> PromoteAsync(string id) {
            ...
            var stack = await _stackRepository.GetAsync(id);
        ...
        private async Task<ActionResult<IReadOnlyCollection<Stack>>> GetInternalAsync(AppFilter sf, TimeInfo ti, string filter = null, string sort = null, string mode = null, int page = 1, int limit = 10) {
            ...
            try {
                var results = await _repository.QueryAsync(q => q.AppFilter(ShouldApplySystemFilter(sf, filter) ? sf : null).FilterExpression(filter).SortExpression(sort).DateRange(ti.Range.UtcStart, ti.Range.UtcEnd, ti.Field), o => o.PageNumber(page).PageLimit(limit));
        ...
        private async Task<ActionResult<IReadOnlyCollection<Stack>>> GetAllByTermsAsync(string aggregations, AppFilter sf, TimeInfo ti, string filter = null, string mode = null, int page = 1, int limit = 10) {
            page = GetPage(page);
            limit = GetLimit(limit);
            int skip = GetSkip(page, limit);
            if (skip > MAXIMUM_SKIP)
        ...
        private async Task<ICollection<StackSummaryModel>> GetStackSummariesAsync(ICollection<Stack> stacks, AppFilter eventSystemFilter, TimeInfo ti) {
            ...
            var stackTerms = await _eventRepository.CountByQueryAsync(q => q.SystemFilter(systemFilter).FilterExpression(String.Join(" OR ", stacks.Select(r => $"stack:{r.Id}"))).AggregationsExpression($"terms:(stack_id~{stacks.Count} cardinality:user sum:count~1 min:date max:date)"));
        ...
        private async Task<Dictionary<string, double>> GetUserCountByProjectIdsAsync(ICollection<Stack> stacks, AppFilter sf, DateTime utcStart, DateTime utcEnd) {
            ...
            var countResult = await _eventRepository.CountByQueryAsync(q => q.SystemFilter(systemFilter).FilterExpression(projects.BuildFilter()).AggregationsExpression("terms:(project_id cardinality:user)"));
