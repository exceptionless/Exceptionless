    ...
    public class StackController : RepositoryApiController<IStackRepository, Stack, Stack, Stack, Stack> {
        ...
        private async Task<ActionResult<IReadOnlyCollection<Stack>>> GetAllByTermsAsync(string aggregations, AppFilter sf, TimeInfo ti, string filter = null, string mode = null, int page = 1, int limit = 10) {
            ...
            try {
                ...
                var stackTerms = (await _eventRepository.CountAsync(q => q.SystemFilter(systemFilter).FilterExpression(filter).AggregationsExpression($"terms:(stack_id~{GetSkip(page + 1, limit) + 1} {aggregations})"))).Aggregations.Terms<string>("terms_stack_id");
