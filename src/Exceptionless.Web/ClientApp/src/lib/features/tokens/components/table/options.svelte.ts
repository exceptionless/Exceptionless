...
export function getTableContext<TToken extends ViewToken>(
    params: GetProjectTokensParams,
    configureOptions: (options: TableOptions<TToken>) => TableOptions<TToken> = (options) => options
) {
    let _parameters = $state(params);
    let _pageCount = $state(0);
    let _columns = $state(getColumns<TToken>());
    let _data = $state([] as TToken[]);
    let _loading = $state(false);
    let _meta = $state({} as FetchClientResponse<unknown>['meta']);

    const [columnVisibility, setColumnVisibility] = createPersistedTableState('token-column-visibility', <VisibilityState>{});
