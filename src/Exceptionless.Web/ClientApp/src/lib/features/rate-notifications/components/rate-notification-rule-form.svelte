<script lang="ts">
    ...
    let { onCancel, onSaved, projectId, rule, userId }: Props = $props();
    ...
    const form = createForm(() => ({
        ...
        validators: {
            ...
            onSubmitAsync: async ({ value }) => {
                ...
                try {
                    const request = toRateNotificationRuleRequest(value);
                    const route = {
                        projectId,
                        userId
                    };
    ...
    function setFormValues(value: undefined | ViewRateNotificationRule) {
        ...
        form.setFieldValue('is_enabled', values.is_enabled);
