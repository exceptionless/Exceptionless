<script lang="ts">
    ...
    let { onCreateClick, onEditClick, projectId, userId }: Props = $props();
...
<div class="flex flex-col gap-4">
    ...
    {:else}
                ...
                <div class="rounded-lg border p-4">
                    <div class="flex items-start justify-between gap-3">
                        <div class="min-w-0 flex-1">
                            <Button variant="link" onclick={() => onEditClick?.(rule)}>{rule.name}</Button>
                            <div class="mt-1 flex flex-wrap items-center gap-2">
                                ...
                                {#if rule.last_fired_utc}
                                    <Muted class="text-xs">Last triggered <TimeAgo value={rule.last_fired_utc} /></Muted>
                                {/if}
                            </div>
                        </div>
        ...
        {#if rules.length < MAX_RULES_PER_PROJECT}
            <Button variant="outline" size="sm" onclick={onCreateClick}>
                <PlusIcon data-icon="inline-start" aria-hidden="true" />
                Add rule
            </Button>
