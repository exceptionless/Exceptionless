<script lang="ts">
    ...
    let { selectedIds, onActionsComplete }: Props = $props();
    ...
    async function markOpen() {
        ...
        if (selectedIds.length === 1) {
            ...
            toast.success(`Successfully marked ${Intl.NumberFormat().format(selectedIds.length)} stacks as open.`);
        }
    ...
    async function markFixed(version?: string) {
        ...
        if (selectedIds.length === 1) {
            ...
            toast.success(`Successfully marked ${Intl.NumberFormat().format(selectedIds.length)} stacks as fixed.`);
        }
    ...
    async function markSnoozed(timePeriod?: '6hours' | 'day' | 'month' | 'week') {
        ...
        if (selectedIds.length === 1) {
            ...
            toast.success(`Successfully marked ${Intl.NumberFormat().format(selectedIds.length)} stacks as snoozed.`);
        }
    ...
    async function markIgnored() {
        ...
        if (selectedIds.length === 1) {
            ...
            toast.success(`Successfully marked ${Intl.NumberFormat().format(selectedIds.length)} stacks as ignored.`);
        }
    ...
    async function markDiscarded() {
        ...
        if (selectedIds.length === 1) {
            ...
            toast.success(`Successfully marked ${Intl.NumberFormat().format(selectedIds.length)} stacks as discarded.`);
        }
    ...
    async function remove() {
        ...
        if (selectedIds.length === 1) {
            ...
            toast.success(`Successfully deleted ${Intl.NumberFormat().format(selectedIds.length)} stacks.`);
        }
...
{#if openMarkStackDiscardedDialog}
    <MarkStackDiscardedDialog bind:open={openMarkStackDiscardedDialog} discard={markDiscarded} count={selectedIds.length} />
{/if}
{#if openMarkStackFixedInVersionDialog}
    <MarkStackFixedInVersionDialog bind:open={openMarkStackFixedInVersionDialog} save={markFixed} count={selectedIds.length} />
{/if}
{#if openRemoveStackDialog}
    <RemoveStackDialog bind:open={openRemoveStackDialog} {remove} count={selectedIds.length} />
{/if}
