<script lang="ts">
    ...
    import FacetedFilterBuilder from './faceted-filter-builder.svelte';

    let { changed, filters, remove }: { changed: (filter: IFilter) => void; filters: IFilter[]; remove: (filter?: IFilter) => void } = $props();
</script>
