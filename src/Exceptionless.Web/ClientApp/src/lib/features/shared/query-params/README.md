...
- cancellation of pending history-entry coalescing during navigation and component teardown.
...
URL synchronization is synchronous: once query-parameter state changes, a reload observes the same state. With `history: 'push'`, the first update immediately creates a history entry and rapid follow-up updates replace that entry until `debounceMilliseconds` elapses. This keeps the URL durable without producing a Back-button entry for every keystroke. With `history: 'replace'`, every update immediately replaces the current entry.
