...
<EmailLayout styles={actionsFooterStyles}>
    {#snippet content()}
        <Section data-email-content class="px-4 py-2">
            <Heading as="h1" class="text-foreground mt-0 mb-[5px] text-[34px] leading-[1.3] font-normal">Payment Failed</Heading>
        ...
        <ActionsFooter>
            {#snippet actions()}
                <li class="mt-[5px] ml-[5px]">
                    <Link href="{'{{BaseUrl}}'}/organization/{'{{OrganizationId}}'}/manage?tab=billing" class="text-primary no-underline">View invoices</Link>
                </li>
            {/snippet}
        </ActionsFooter>
    {/snippet}
