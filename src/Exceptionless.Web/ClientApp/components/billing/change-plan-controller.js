        ...
        .controller(
            ...
            function (
                ...
                function cancel() {
                    ...
                    $ExceptionlessClient
                        .createFeatureUsage(vm._source + ".cancel")
                ...
                function getAnalyticsData() {
                    return {
                        content_name: vm.currentPlan.name,
                        content_ids: [vm.currentPlan.id],
                        content_type: "product",
                        currency: "USD",
                        value: vm.currentPlan.price,
                    };
                }

                function save(isValid) {
                    function onCreateTokenSuccess(response) {
                        return changePlan(false, {
                    ...
                    function onSuccess(response) {
                        if (!response.data.success) {
                            analyticsService.lead(getAnalyticsData());
                            vm.paymentMessage =
                                translateService.T("An error occurred while changing plans.") +
                                " " +
                                translateService.T("Message:") +
                                " " +
                                response.data.message;
                            $ExceptionlessClient
                                .createException(new Error(response.data.message))
                                .markAsCritical()
                    ...
                    function onFailure(response) {
                        ...
                        analyticsService.lead(getAnalyticsData());
                        $ExceptionlessClient
                            .createException(new Error(vm.paymentMessage))
                            .markAsCritical()
                            .setSource(vm._source + ".save.error")
                            .setProperty("CurrentPlan", vm.currentPlan)
                            .setProperty("CouponId", vm.coupon)
                ...
                function changePlan(isAdmin, options) {
                    if (isAdmin) {
                        return adminService.changePlan({
                            organizationId: vm.currentOrganization.id,
                ...
                function getOrganizations() {
                    ...
                    function onSuccess() {
                        vm.currentOrganization = vm.organizations.filter(function (o) {
                            return o.id === (vm.currentOrganization.id || data);
                        })[0];
                ...
                function getPlans() {
                    function onSuccess(response) {
                        ...
                        var currentPlan =
                            vm.plans.filter(function (p) {
                                return p.id === vm.currentOrganization.plan_id;
                            })[0] || vm.plans[0];
                        var currentPlanIndex = vm.plans.indexOf(currentPlan);
                        vm.currentPlan =
                            vm.plans.length > currentPlanIndex + 1 ? vm.plans[currentPlanIndex + 1] : currentPlan;
                ...
                function isCancellingPlan() {
                    return (
                        vm.currentPlan &&
                        vm.currentPlan.id === vm._freePlanId &&
                        vm.currentOrganization.plan_id !== vm._freePlanId
                    );
                }
                ...
                this.$onInit = function $onInit() {
                    ...
                    vm.organizations = [];
                    vm.paymentMessage = !isBillingEnabled()
                        ? translateService.T("Billing is currently disabled.")
                        : null;
                    vm.plans = [];
                    vm.save = save;
