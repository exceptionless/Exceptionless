(function () {
    ...
    angular.module("exceptionless.events").directive("events", function (linkService) {
        return {
            ...
            controller: [
                ...
                function (
                    ...
                    function afterRelativeText(ev) {
                        return moment(ev.date).diff(vm.settings.relativeTo, "milliseconds") >= 0
                            ? $translate.instant("after") + " "
                            : "";
                    }

                    function beforeRelativeText(ev) {
                        return moment(ev.date).diff(vm.settings.relativeTo, "milliseconds") < 0
                            ? " " + $translate.instant("before")
                            : "";
                    }
                    ...
                    function get(options) {
                        function onSuccess(response) {
                            ...
                            vm.selectedIds = vm.selectedIds.filter(function (id) {
                                return (
                                    vm.events.filter(function (e) {
                                        return e.id === id;
                                    }).length > 0
                        ...
                        function onFailure(response) {
                            ...
                            $ExceptionlessClient
                                .createLog(vm._source + ".get", "Error while loading events", "Error")
                    ...
                    function open(id, event) {
                        ...
                        $ExceptionlessClient
                            .createFeatureUsage(vm._source + ".open")
                            .setProperty("id", id)
                            .setProperty("_blank", openInNewTab)
                            .submit();
                        if (openInNewTab) {
