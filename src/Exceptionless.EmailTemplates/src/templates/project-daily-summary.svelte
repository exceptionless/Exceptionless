...
<EmailLayout>
  {#snippet content()}
    <Section class="py-2 px-4">
      <Heading as="h1" class="text-[34px] font-normal text-[#2c2c2c] leading-[1.3] mt-0 mb-[5px]">{@html 'Summary for {{StartDate}}'}</Heading>
      ...
      <Text class="text-base text-[#2c2c2c] leading-[1.3]">{@html '{{#if Count}}The "{{ProjectName}}" project had <strong>{{Count}} total</strong>, <strong>{{Unique}} unique</strong>, and <strong>{{New}} new</strong> errors.{{else}}Congrats! The "{{ProjectName}}" project was exceptionless!{{/if}}{{#if Fixed}} Additionally, <strong>{{Fixed}} errors</strong> that have been marked as fixed occurred in outdated instances of your application.{{/if}}'}</Text>

      <Section class="text-center">
        <Button href="{'{{BaseUrl}}'}/project/{'{{ProjectId}}'}/error/timeline" class="bg-[#5E9A00] text-white font-bold text-base rounded-[3px] px-4 py-2 no-underline inline-block">View Timeline</Button>
      </Section>

      {@html '{{#if Blocked}}'}
      <Section class="border border-[#BB423F] bg-[#f4dede] p-[10px] my-4 rounded-[3px]">
        <Text class="text-base text-[#2c2c2c] leading-[1.3]">{@html '<strong>{{Blocked}} events</strong> were discarded due to throttling.'} <Link href="{'{{BaseUrl}}'}/organization/{'{{OrganizationId}}'}/upgrade" class="text-[#5E9A00] no-underline">Upgrade now</Link> to increase your limits. <Link href="https://github.com/exceptionless/Exceptionless/wiki/Frequently-Asked-Questions#q-why-is-my-organization-throttled" class="text-[#5E9A00] no-underline">Click here to learn more about throttling.</Link></Text>
        <Section class="text-center">
          <Button href="{'{{BaseUrl}}'}/organization/{'{{OrganizationId}}'}/upgrade" class="bg-[#BB423F] text-white font-bold text-base rounded-[3px] px-4 py-2 no-underline inline-block">Upgrade Plan</Button>
        </Section>
      </Section>
      {@html '{{/if}}'}

      {@html '{{#if MostFrequent}}'}
      <Heading as="h5" class="text-[20px] font-normal text-[#939393] leading-[1.3] mt-0 mb-[5px]">Most Frequent</Heading>
      ...
      {@html '{{#if Newest}}'}
      <Heading as="h5" class="text-[20px] font-normal text-[#939393] leading-[1.3] mt-0 mb-[5px]">Newest</Heading>
      {@html '{{#each Newest}}'}
      {@html '<ul style="margin-top:0"><li style="margin-top:5px;margin-left:5px"><a href="{{../BaseUrl}}/stack/{{StackId}}" style="color:#5E9A00;text-decoration:none">{{#if IsRegressed}}<strong>[REGRESSED]</strong> {{/if}}{{#if TypeName}}<strong>{{TypeName}}:</strong> {{/if}}{{Title}}</a></li></ul>'}
      {@html '{{/each}}'}
      ...
      {@html '{{#if IsFreePlan}}'}
      <Text class="text-base text-[#2c2c2c] leading-[1.3]">You are currently on a free plan. If you would like to receive notifications for errors as they happen, <Link href="{'{{BaseUrl}}'}/organization/{'{{OrganizationId}}'}/upgrade" class="text-[#5E9A00] no-underline">upgrade to a paid plan</Link>.</Text>
      {@html '{{/if}}'}

      {@html '{{else}}'}
      <Text class="text-[20px] leading-[1.6] text-[#2c2c2c]">{@html 'Unfortunately, it appears that your "{{ProjectName}}" project has not yet been configured to send errors to'} <Link href="https://exceptionless.com" class="text-[#5E9A00] no-underline">Exceptionless</Link>.</Text>
      <Section class="text-center">
        <Button href="{'{{BaseUrl}}'}/project/{'{{ProjectId}}'}/configure" class="bg-[#5E9A00] text-white font-bold text-base rounded-[3px] px-4 py-2 no-underline inline-block">Configure Project</Button>
      </Section>
      <Text class="text-base text-[#2c2c2c] leading-[1.3]">Send us an email at <Link href="mailto:support@exceptionless.io" class="text-[#5E9A00] no-underline">support@exceptionless.io</Link> if you have any questions or need help getting started.</Text>
      {@html '{{/if}}'}
    </Section>

    <ActionsFooter>
      {#snippet actions()}
        <li class="mt-[5px] ml-[5px]"><Link href="{'{{BaseUrl}}'}/account/manage?projectId={'{{ProjectId}}'}&tab=notifications" class="text-[#6EBC1A] no-underline">Change your notification settings for this project</Link></li>
      {/snippet}
    </ActionsFooter>
  {/snippet}
</EmailLayout>
