        ...
        protected override async Task<JobResult> RunInternalAsync(JobContext context) {
            ...
            while (true) {
                ...
                foreach (var task in workingTasks.ToArray()) {
                    ...
                    if (!taskStatus.IsValid) {
                        ...
                        if (task.Errors.Count > 5 || taskStatus.Completed)
                        {
                            ...
                            if (taskStatus.Completed && isConnectionError) {
                                _logger.LogWarning("Reindexing Failed and will be retried ({TaskId}) {TargetIndex} [Duration: {Duration:hh\\:mm} - {Progress:P}] - Created: {Created} Updated: {Updated} Deleted: {Deleted} Conflicts: {Conflicts} Total: {Total}", task.TaskId, task.TargetIndex, duration, progress, status.Created, status.Updated, status.Deleted, status.VersionConflicts, status.Total);
                                ...
                                _logger.LogCritical("Reindexing Failed ({TaskId}) {TargetIndex} [Duration: {Duration:hh\\:mm} - {Progress:P}] - Created: {Created} Updated: {Updated} Deleted: {Deleted} Conflicts: {Conflicts} Total: {Total}", task.TaskId, task.TargetIndex, duration, progress, status.Created, status.Updated, status.Deleted, status.VersionConflicts, status.Total);
                            }
                        }
                    ...
                    _logger.LogInformation("Reindex completed ({TaskId}) [{Duration:hh\\:mm} - {Progress:P}]: {SourceIndex}: {SourceCount} {TargetIndex}: {TargetCount} - Created: {Created} Updated: {Updated} Deleted: {Deleted} Conflicts: {Conflicts} Total: {Total}", task.TaskId, duration, progress, task.SourceIndex, sourceCount.Count, task.TargetIndex, targetCount.Count, status.Created, status.Updated, status.Deleted, status.VersionConflicts, status.Total);
                }
                if (SystemClock.UtcNow.Subtract(lastProgress) > TimeSpan.FromMinutes(5)) {
                    _logger.LogInformation("Data migration - C:{Completed}/{Total} W:{Working} P:{Progress:P} D:{Duration:d\\.hh\\:mm} Failed:{Failed}", completedTasks.Count, totalTasks, workingTasks.Count, highestProgress, SystemClock.UtcNow.Subtract(started), failedTasks.Count);
            ...
            foreach (var task in completedTasks) {
                ...
                var sourceCount = await client.CountAsync<object>(d => d.Index(task.SourceIndex)).AnyContext();
                var targetCount = await client.CountAsync<object>(d => d.Index(task.TargetIndex)).AnyContext();
                _logger.LogInformation("Reindex completed ({TaskId}) [Duration: {Duration:hh\\:mm} - {Progress:P}]: {SourceIndex}/{SourceType}: {SourceCount} {TargetIndex}: {TargetCount} - Created: {Created} Updated: {Updated} Deleted: {Deleted} Conflicts: {Conflicts} Total: {Total}", task.TaskId, duration, progress, task.SourceIndex, task.SourceIndexType, sourceCount.Count, task.TargetIndex, targetCount.Count, status.Created, status.Updated, status.Deleted, status.VersionConflicts, status.Total);
            }
            ...
            foreach (var task in failedTasks) {
                ...
                _logger.LogCritical("Reindex failed ({TaskId}) [Duration: {Duration:hh\\:mm} - {Progress:P}]: {SourceIndex}: {SourceCount} {TargetIndex}: {TargetCount} - Created: {Created} Updated: {Updated} Deleted: {Deleted} Conflicts: {Conflicts} Total: {Total}", task.TaskId, duration, progress, task.SourceIndex, sourceCount.Count, task.TargetIndex, targetCount.Count, status.Created, status.Updated, status.Deleted, status.VersionConflicts, status.Total);
            }
