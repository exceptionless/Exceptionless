...
public class SerializerTests : TestWithServices
{
    ...
    [Fact]
    public void SerializeToString_HtmlSensitiveCharacters_EscapesDangerousCharacters()
    {
        // Arrange
        var payload = new { Text = "<script>alert('x') & \"日本語\"" };

        // Act
        string json = _serializer.SerializeToString(payload);

        // Assert
        Assert.Equal("""{"text":"\u003Cscript\u003Ealert(\u0027x\u0027) \u0026 \u0022日本語\u0022"}""", json);
    }
    ...
    public void Deserialize_EventWithData_PreservesDataValues()
    {
        ...
        Assert.Equal("SomeVal", data["Blah"]);
    }

    [Fact]
    public async Task Deserialize_EventWithUnknownProperties_CapturesAllValues()
    {
    ...
    [Fact]
    public void RoundTrip_WebHook_PreservesValues()
    {
    ...
    [Fact]
    public void Deserialize_Project_ParsesDatesCorrectly()
    {
        /* language=json */
    ...
    [Theory]
    [InlineData("""{"data":{"@error":{"message":"DataError","type":"System.ArgumentException","stack_trace":[]}},"@error":{"message":"RootError","type":"System.InvalidOperationException","stack_trace":[]}}""")]
    [InlineData("""{"@error":{"message":"RootError","type":"System.InvalidOperationException","stack_trace":[]},"data":{"@error":{"message":"DataError","type":"System.ArgumentException","stack_trace":[]}}}""")]
    public void OnDeserialized_DataAndRootLevelCollision_PreservesBothValues(string json)
    {
        // Arrange — legacy DataObjectConverter preserved duplicate data keys by renaming
        // the later duplicate instead of dropping one of the user-provided values.
        ...
        Assert.NotNull(ev.Data);
        Assert.True(ev.Data.ContainsKey(Event.KnownDataKeys.Error));
        Assert.True(ev.Data.ContainsKey("_@error"));

        var error = ev.Data.GetValue<Error>(Event.KnownDataKeys.Error, _serializer);
        var duplicateError = ev.Data.GetValue<Error>("_@error", _serializer);

        Assert.NotNull(error);
        Assert.NotNull(duplicateError);
        Assert.Equal("DataError", error.Message);
        Assert.Equal("RootError", duplicateError.Message);
    }
