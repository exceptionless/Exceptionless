...
public class EventStackFilterQueryTests : IntegrationTestsBase
{
    ...
    protected override async Task ResetDataAsync()
    {
        ...
        await CreateDataAsync(d =>
        {
            ...
            d.Event().Type(Event.KnownTypes.Log).Status(StackStatus.Open).Reference("parent", "parentReferenceId");
    ...
    [InlineData("project:" + SampleDataService.TEST_PROJECT_ID + " (status:open OR status:regressed)", 7, 4)]
    ...
    [InlineData("is_regressed:true", 1)]
    [InlineData("is_hidden:true", 4)]
    [InlineData("project:" + SampleDataService.TEST_PROJECT_ID + " (status:open OR status:regressed)", 6)]
    ...
    [InlineData("project:" + SampleDataService.TEST_PROJECT_ID + " (status:open OR status:regressed) ref.parent:parentReferenceIdenceId", 1)]
