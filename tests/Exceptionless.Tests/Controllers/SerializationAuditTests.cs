...
public sealed class SerializationAuditTests : IntegrationTestsBase
{
    ...
    private Elasticsearch.Net.IElasticLowLevelClient? _lowLevelClient;
    ...
    public SerializationAuditTests(ITestOutputHelper output, AppWebHostFactory factory) : base(output, factory)
    {
        ...
        _lowLevelClient = _esConfiguration.Client.LowLevel;
    }
    ...
    /// <summary>
    /// Get the raw JSON document from Elasticsearch by ID using Nest's low-level API.
    /// Works with main branch (Nest v7 / IElasticClient).
    /// </summary>
    private async Task<string> GetElasticsearchDocumentAsync(string indexPattern, string id)
    {
        var llc = _lowLevelClient!;

        // Step 1: resolve wildcard pattern to concrete indices via _cat/indices
        // (GET /{wildcard}/{id} does not work — GET requires a concrete index)
        string concreteIndex = await ResolveConcreteIndexAsync(llc, indexPattern, id);

        // Step 2: direct GET on the concrete index
        var getResponse = await llc.GetAsync<StringResponse>(concreteIndex, id);
        if (getResponse.Success && getResponse.Body is not null)
        {
            try
            {
                var doc = JsonDocument.Parse(getResponse.Body);
                if (doc.RootElement.TryGetProperty("_source", out var source))
                    return PrettyPrint(JsonSerializer.Serialize(source));
                return PrettyPrint(getResponse.Body);
            }
            catch { return PrettyPrint(getResponse.Body); }
        }

        // Step 3: fallback search
        string searchJson = "{\"query\":{\"ids\":{\"values\":[\"" + id + "\"]}},\"size\":1}";
        var searchResponse = await llc.SearchAsync<StringResponse>(
            concreteIndex,
            Elasticsearch.Net.PostData.String(searchJson));
        if (searchResponse.Success && searchResponse.Body is not null)
        {
            try
            {
                var doc = JsonDocument.Parse(searchResponse.Body);
                var hits = doc.RootElement.GetProperty("hits").GetProperty("hits");
                if (hits.GetArrayLength() > 0 && hits[0].TryGetProperty("_source", out var source))
                    return PrettyPrint(JsonSerializer.Serialize(source));
            }
            catch { /* fall through */ }
        }

        return $"{{\"error\": \"Document {id} not found in {indexPattern}\"}}";
    }

    private async Task<string> ResolveConcreteIndexAsync(Elasticsearch.Net.IElasticLowLevelClient llc, string indexPattern, string id)
    {
        // If already concrete (no wildcards), return as-is
        if (!indexPattern.Contains('*') && !indexPattern.Contains('?'))
            return indexPattern;

        // Use search across the pattern to find which index holds the document
        string searchJson = "{\"query\":{\"ids\":{\"values\":[\"" + id + "\"]}},\"size\":1,\"_source\":false}";
        var resp = await llc.SearchAsync<StringResponse>(
            indexPattern,
            Elasticsearch.Net.PostData.String(searchJson),
            new Elasticsearch.Net.SearchRequestParameters
            {
        ...
        if (resp.Success && resp.Body is not null)
        {
            try
            {
                ...
                if (hits.GetArrayLength() > 0)
                    return hits[0].GetProperty("_index").GetString() ?? indexPattern;
            }
            catch { /* fall through */ }
        }
