...
{
    [Fact]
    public void Deserialize_SharingDefaultWithoutEnabled_RejectsMissingChoice()
    {
        Assert.Throws<JsonException>(() => JsonSerializer.Deserialize<UpdateAssistantConversationSharingSettings>("{}", JsonSerializerOptions.Web));
    }

    [Theory]
    [InlineData(true)]
    [InlineData(false)]
    public void Deserialize_SharingDefaultWithEnabled_PreservesChoice(bool enabled)
    {
        string json = JsonSerializer.Serialize(new { enabled });
        var settings = JsonSerializer.Deserialize<UpdateAssistantConversationSharingSettings>(json, JsonSerializerOptions.Web);
