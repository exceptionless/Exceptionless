...
public sealed class AssistantDiagnosticsTests
{
    ...
    public void RecordException_CancellationSource_RecordsExpectedProviderOutcome(string source, string expectedOutcome)
    {
        ...
        using (var provider = diagnostics.StartProviderRequest(deadline.Token))
        {
            if (source == "client")
            {
    ...
    public void ObserveChunk_OutputLimit_RecordsGenerationAndReasoningWithoutContent()
    {
        ...
        using var provider = diagnostics.StartProviderRequest(TestContext.Current.CancellationToken);
    ...
    [Fact]
    public void StartTool_AfterFailedTool_ClearsPreviousError()
    {
        var logger = new RecordingAssistantLogger();
        using var diagnostics = new AssistantTurnDiagnostics(logger, TimeProvider.System, "organization-id", "conversation-id", "request-id");
        diagnostics.StartTool("search_stacks");
        diagnostics.RecordToolResult("""{"ok":false,"error":{"code":"invalid_filter"}}""", 100);

        diagnostics.StartTool("get_event");
        diagnostics.RecordToolResult("""{"ok":true}""", 50);
        diagnostics.Finish("completed");

        Assert.Equal("get_event", diagnostics.LastTool);
        Assert.Null(diagnostics.LastToolError);
        Assert.Equal(1, diagnostics.ToolFailures);
        Assert.Null(logger.Entries[^1].Properties["LastToolError"]);
    }
