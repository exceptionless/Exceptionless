...
public sealed class EventValidatorTests : TestWithServices
{
    ...
    {
        var ev = new PersistentEvent { Type = Event.KnownTypes.Error, Date = _timeProvider.GetLocalNow(), Id = "123456789012345678901234", OrganizationId = "123456789012345678901234", ProjectId = "123456789012345678901234", StackId = "123456789012345678901234" };
        ev.Tags ??= new TagSet();
        ev.Tags.Add(tag);
    ...
    {
        var result = _validator.Validate(new PersistentEvent { Type = Event.KnownTypes.Error, ReferenceId = referenceId, Date = _timeProvider.GetLocalNow(), Id = "123456789012345678901234", OrganizationId = "123456789012345678901234", ProjectId = "123456789012345678901234", StackId = "123456789012345678901234" });
        Assert.Equal(isValid, result.IsValid);
    }
    ...
    {
        var date = minutes.HasValue ? _timeProvider.GetLocalNow().AddMinutes(minutes.Value) : DateTimeOffset.MinValue;
    ...
    {
        var result = _validator.Validate(new PersistentEvent { Type = type, Date = _timeProvider.GetLocalNow(), Id = "123456789012345678901234", OrganizationId = "123456789012345678901234", ProjectId = "123456789012345678901234", StackId = "123456789012345678901234" });
        Assert.Equal(isValid, result.IsValid);
    }
}
