...
public class CleanupOrphanedDataJobTests : IntegrationTestsBase
{
    ...
    {
        // Arrange - Multiple valid stacks in two orgs, no orphans
        var organization1 = _organizationData.GenerateOrganization(_billingManager, _plans, id: TestConstants.OrganizationId);
        var organization2 = _organizationData.GenerateOrganization(_billingManager, _plans, id: TestConstants.OrganizationId2);
        await _organizationRepository.AddAsync([organization1, organization2], o => o.ImmediateConsistency());
    ...
    public async Task DeleteOrphanedEventsByProject_WithOrphanedProject_DeletesEventsForMissingProject()
    {
        ...
        // Orphaned events referencing a non-existent project in org 2
    ...
    public async Task DeleteOrphanedEventsByOrganization_WithValidOrganizations_DoesNotDeleteEvents()
    {
        ...
        // Assert - All events preserved (both orgs exist)
        var totalAfter = await _eventRepository.CountAsync(o => o.IncludeSoftDeletes().ImmediateConsistency());
        Assert.Equal(160, totalAfter);
    }

    [Fact]
    public async Task DeleteOrphanedEventsByOrganization_WithOrphanedOrganization_DeletesEventsForMissingOrg()
    {
        // Arrange - Valid org1 + events referencing non-existent org
        var organization1 = _organizationData.GenerateOrganization(_billingManager, _plans, id: TestConstants.OrganizationId);
        ...
        string fakeProjectId = ObjectId.GenerateNewId().ToString();
        string fakeStackId = ObjectId.GenerateNewId().ToString();
        var orphanedEvents = _eventData.GenerateEvents(50, fakeOrgId, fakeProjectId, fakeStackId).ToList();
    ...
    {
        // Arrange - Org 1 exists, Org 2 does NOT exist (never created) but has events
        var organization1 = _organizationData.GenerateOrganization(_billingManager, _plans, id: TestConstants.OrganizationId);
        await _organizationRepository.AddAsync(organization1, o => o.ImmediateConsistency());
        ...
        // Tenant 2 events (org doesn't exist, simulates post-hard-delete orphans)
        string ghostOrgId = ObjectId.GenerateNewId().ToString();
        string ghostProjectId = ObjectId.GenerateNewId().ToString();
        string ghostStackId = ObjectId.GenerateNewId().ToString();
        var ghostEvents = _eventData.GenerateEvents(80, ghostOrgId, ghostProjectId, ghostStackId).ToList();
    ...
    {
        // Arrange - Complex scenario: valid data, orphaned by stack, orphaned by project, orphaned by org
        var organization1 = _organizationData.GenerateOrganization(_billingManager, _plans, id: TestConstants.OrganizationId);
        await _organizationRepository.AddAsync(organization1, o => o.ImmediateConsistency());
        ...
        // 25 orphaned by organization (org doesn't exist)
        string fakeOrg = ObjectId.GenerateNewId().ToString();
        string fakeProject2 = ObjectId.GenerateNewId().ToString();
        string fakeStack3 = ObjectId.GenerateNewId().ToString();
        var orphanedByOrg = _eventData.GenerateEvents(25, fakeOrg, fakeProject2, fakeStack3).ToList();

        await _eventRepository.AddAsync(validEvents.Concat(orphanedByStack).Concat(orphanedByProject).Concat(orphanedByOrg), o => o.ImmediateConsistency());
