...
public class CleanupDataJobTests : IntegrationTestsBase
{
    ...
    [Fact]
    public async Task CanCleanupSuspendedTokens_MultiTenant_OnlySuspendedOrgTokensAffected()
    {
        // Arrange - Org 1 is suspended, Org 2 is active
        ...
        // Assert - Only Org 1's token is suspended
    ...
    {
        // Arrange - Org suspended, token already marked as suspended
    ...
    [Fact]
    public async Task CanCleanupSuspendedTokens_MultipleTokensPerOrg_AllGetSuspended()
    {
        // Arrange - Suspended org with many tokens
        var organization = _organizationData.GenerateOrganization(_billingManager, _plans, id: TestConstants.OrganizationId);
        organization.IsSuspended = true;
        organization.SuspensionDate = DateTime.UtcNow;
        organization.SuspendedByUserId = TestConstants.UserId;
    ...
    [Fact]
    public async Task CanCleanupSuspendedTokens_NoSuspendedOrgs_NoTokensModified()
    {
        // Arrange - Two active orgs with tokens
        var organization1 = _organizationData.GenerateOrganization(_billingManager, _plans, id: TestConstants.OrganizationId);
        var organization2 = _organizationData.GenerateOrganization(_billingManager, _plans, id: TestConstants.OrganizationId2);
    ...
    [Fact]
    public async Task CanCleanupSoftDeletedOrganization_MultiTenant_OnlyDeletedOrgCleaned()
    {
        // Arrange - Org 1 is soft-deleted, Org 2 is active
        var organization1 = _organizationData.GenerateOrganization(_billingManager, _plans, id: TestConstants.OrganizationId);
        ...
        // Assert - Org 1's entire hierarchy is hard-deleted; Org 2's everything remains
    ...
    {
        // Arrange - Two orgs, Org 1 project is soft-deleted, Org 2 project is active
        ...
        // Assert - Project 1's stacks/events gone; Project 2 and both orgs remain
    ...
    {
        // Arrange - Same org, two projects, one stack soft-deleted in project 1
    ...
    {
        // Arrange - Two orgs on free plan, each has events inside and outside retention
        var organization1 = _organizationData.GenerateOrganization(_billingManager, _plans, id: TestConstants.OrganizationId);
        ...
        // Org 1: 1 recent (keep) + 1 at retention boundary (will be expired after time advance)
        ...
        // Org 2: 1 recent (keep) + 1 at retention boundary
    ...
    {
        // Arrange - Org 1 on free plan (short retention), Org 2 on unlimited (long retention)
        var organization1 = _organizationData.GenerateOrganization(_billingManager, _plans, id: TestConstants.OrganizationId);
        _billingManager.ApplyBillingPlan(organization1, _plans.FreePlan);
        var organization2 = _organizationData.GenerateOrganization(_billingManager, _plans, id: TestConstants.OrganizationId2);
    ...
    {
        // Arrange - Complex scenario: suspended org, soft-deleted project, soft-deleted stack, retention
        // Org 1: suspended (tokens get suspended)
        var organization1 = _organizationData.GenerateOrganization(_billingManager, _plans, id: TestConstants.OrganizationId);
        organization1.IsSuspended = true;
        organization1.SuspensionDate = DateTime.UtcNow;
        organization1.SuspendedByUserId = TestConstants.UserId;
        ...
        // Org 2: has a soft-deleted project (project and children get cleaned up)
        var organization2 = _organizationData.GenerateOrganization(_billingManager, _plans, id: TestConstants.OrganizationId2);
        await _organizationRepository.AddAsync(organization2, o => o.ImmediateConsistency());

        // Org 1 token (should become suspended)
        var token1 = _tokenData.GenerateToken(generateId: true, organizationId: organization1.Id, projectId: TestConstants.ProjectId);
        await _tokenRepository.AddAsync(token1, o => o.ImmediateConsistency());

        // Org 2 token (should remain unsuspended)
        var token2 = _tokenData.GenerateToken(generateId: true, organizationId: organization2.Id, projectId: TestConstants.ProjectIdWithNoRoles);
        await _tokenRepository.AddAsync(token2, o => o.ImmediateConsistency());

        // Project 1 for Org 1 (active project, suspended org)
        var project1 = await _projectRepository.AddAsync(_projectData.GenerateProject(id: TestConstants.ProjectId, organizationId: organization1.Id), o => o.ImmediateConsistency());

        // Project 2 for Org 2 (soft-deleted project)
        ...
        // Project 3 for Org 2 (active project)
        ...
        // Assert
        // Token 1 suspended (org is suspended)
        var updatedToken1 = await _tokenRepository.GetByIdAsync(token1.Id);
        Assert.NotNull(updatedToken1);
        Assert.True(updatedToken1.IsSuspended);

        // Token 2 not suspended (org is active)
        var updatedToken2 = await _tokenRepository.GetByIdAsync(token2.Id);
        Assert.NotNull(updatedToken2);
        Assert.False(updatedToken2.IsSuspended);

        // Org 1 exists (suspended != deleted), its project/stack/event remain
        Assert.NotNull(await _organizationRepository.GetByIdAsync(organization1.Id));
        ...
        // Org 2 exists, soft-deleted project2 is cleaned up
        Assert.NotNull(await _organizationRepository.GetByIdAsync(organization2.Id));
        ...
        // Org 2's active project3 untouched
    ...
    {
        // Arrange - Two healthy active orgs, no soft deletes, no suspended, no expired
        var organization1 = _organizationData.GenerateOrganization(_billingManager, _plans, id: TestConstants.OrganizationId);
        var organization2 = _organizationData.GenerateOrganization(_billingManager, _plans, id: TestConstants.OrganizationId2);
        await _organizationRepository.AddAsync([organization1, organization2], o => o.ImmediateConsistency());
