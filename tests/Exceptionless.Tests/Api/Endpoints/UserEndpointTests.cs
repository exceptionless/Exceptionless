...
public sealed class UserEndpointTests : IntegrationTestsBase
{
    ...
    [Fact]
    public async Task UpdateCurrentUserProductTourAsync_NewProgress_PersistsAndReturnsCurrentUser()
    {
        var currentUser = await GetTestOrganizationUserAsync();

        var updatedUser = await SendRequestAsAsync<ViewCurrentUser>(r => r
            .Put()
            .AsTestOrganizationUser()
            .AppendPaths("users", "me", "product-tours", "new-ui-overview")
            .Content(new UpdateProductTourProgress { Version = 1, Status = ProductTourStatus.Dismissed })
            .StatusCodeShouldBeOk()
        );

        Assert.NotNull(updatedUser);
        var progress = Assert.Contains("new-ui-overview", updatedUser.ProductTours);
        ...
        Assert.Equal(ProductTourStatus.Dismissed, persistedUser.ProductTours["new-ui-overview"].Status);
    }

    [Fact]
    public async Task UpdateCurrentUserProductTourAsync_SameVersionCompletedThenDismissed_PreservesCompleted()
    {
        ...
        await SendRequestAsync(r => r
            .Put()
            .AsTestOrganizationUser()
            .AppendPaths("users", "me", "product-tours", "create-saved-view")
            .Content(new UpdateProductTourProgress { Version = 2, Status = ProductTourStatus.Completed })
            .StatusCodeShouldBeOk()
        );
        ...
        var updatedUser = await SendRequestAsAsync<ViewCurrentUser>(r => r
            ...
            .AsTestOrganizationUser()
            .AppendPaths("users", "me", "product-tours", "create-saved-view")
            .Content(new UpdateProductTourProgress { Version = 2, Status = ProductTourStatus.Dismissed })
            .StatusCodeShouldBeOk()
        );

        Assert.NotNull(updatedUser);
        Assert.Equal(ProductTourStatus.Completed, updatedUser.ProductTours["create-saved-view"].Status);
    }

    [Fact]
    public async Task UpdateCurrentUserProductTourAsync_LowerVersion_DoesNotOverwriteNewerProgress()
    {
        await GetTestOrganizationUserAsync();
        ...
        await SendRequestAsync(r => r
            .Put()
            .AsTestOrganizationUser()
            .AppendPaths("users", "me", "product-tours", "meet-exie")
            .Content(new UpdateProductTourProgress { Version = 3, Status = ProductTourStatus.Completed })
            .StatusCodeShouldBeOk()
        );
        ...
        Assert.NotNull(updatedUser);
        Assert.Equal(3, updatedUser.ProductTours["meet-exie"].Version);
        Assert.Equal(ProductTourStatus.Completed, updatedUser.ProductTours["meet-exie"].Status);
    }

    [Fact]
    public Task UpdateCurrentUserProductTourAsync_InvalidTourId_ReturnsUnprocessableEntity()
    {
        return SendRequestAsync(r => r
            .Put()
            .AsTestOrganizationUser()
            .AppendPaths("users", "me", "product-tours", "Invalid--Tour")
            .Content(new UpdateProductTourProgress { Version = 1, Status = ProductTourStatus.Completed })
    ...
    [Fact]
    public async Task UpdateCurrentUserProductTourAsync_RepeatedRequest_IsIdempotent()
    {
        const string tourId = "idempotent-guide";
        var first = await SendRequestAsAsync<ViewCurrentUser>(r => r
            .Put()
            .AsTestOrganizationUser()
            .AppendPaths("users", "me", "product-tours", tourId)
            .Content(new UpdateProductTourProgress { Version = 1, Status = ProductTourStatus.Completed })
            .StatusCodeShouldBeOk());
        var second = await SendRequestAsAsync<ViewCurrentUser>(r => r
            .Put()
            .AsTestOrganizationUser()
            .AppendPaths("users", "me", "product-tours", tourId)
            .Content(new UpdateProductTourProgress { Version = 1, Status = ProductTourStatus.Completed })
        ...
        Assert.Equal(first.ProductTours[tourId].UpdatedUtc, second.ProductTours[tourId].UpdatedUtc);
    }

    [Fact]
    public async Task UpdateCurrentUserProductTourAsync_ConcurrentKeys_PreservesBoth()
    {
        Task first = SendRequestAsync(r => r
            .Put()
            .AsTestOrganizationUser()
            .AppendPaths("users", "me", "product-tours", "concurrent-one")
            .Content(new UpdateProductTourProgress { Version = 1, Status = ProductTourStatus.Dismissed })
            .StatusCodeShouldBeOk());
        Task second = SendRequestAsync(r => r
            .Put()
            .AsTestOrganizationUser()
            .AppendPaths("users", "me", "product-tours", "concurrent-two")
            .Content(new UpdateProductTourProgress { Version = 1, Status = ProductTourStatus.Completed })
            .StatusCodeShouldBeOk());

        await Task.WhenAll(first, second);
        var currentUser = await SendRequestAsAsync<ViewCurrentUser>(r => r.AsTestOrganizationUser().AppendPath("users/me").StatusCodeShouldBeOk());
        Assert.NotNull(currentUser);
        Assert.Contains("concurrent-one", currentUser.ProductTours);
        Assert.Contains("concurrent-two", currentUser.ProductTours);
    }

    [Fact]
    public async Task UpdateCurrentUserProductTourAsync_AtEntryLimit_ReturnsUnprocessableEntity()
    {
        var currentUser = await GetTestOrganizationUserAsync();
        var user = await _userRepository.GetByIdAsync(currentUser.Id, o => o.Cache(false));
        Assert.NotNull(user);
        user.ProductTours.Clear();
        foreach (int index in Enumerable.Range(0, 32))
            user.ProductTours[$"guide-{index}"] = new ProductTourProgress { Status = ProductTourStatus.Dismissed, UpdatedUtc = DateTime.UtcNow, Version = 1 };
        await _userRepository.SaveAsync(user, o => o.Cache().ImmediateConsistency());

        await SendRequestAsync(r => r
            .Put()
            .AsTestOrganizationUser()
            .AppendPaths("users", "me", "product-tours", "one-too-many")
            .Content(new UpdateProductTourProgress { Version = 1, Status = ProductTourStatus.Completed })
            .StatusCodeShouldBeUnprocessableEntity());
    }
    ...
    public Task UpdateCurrentUserProductTourAsync_NonPositiveVersion_ReturnsUnprocessableEntity(int version)
    {
        return SendRequestAsync(r => r
            ...
            .Content(new UpdateProductTourProgress { Version = version, Status = ProductTourStatus.Completed })
            .StatusCodeShouldBeUnprocessableEntity());
    }
