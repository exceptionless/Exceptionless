...
- In isolated worktrees, Git metadata and GitHub network checks may be outside the sandbox. If `gh auth status` reports an invalid credential or Git ref writes fail as read-only, retry with elevated access before asking the user to reauthenticate or diagnosing a repository problem.
