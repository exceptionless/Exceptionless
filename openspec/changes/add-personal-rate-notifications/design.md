...
- Store the project plan in the shared cache and coordinate cache fills and invalidation with a distributed per-project lock. This prevents an in-flight query on another replica from publishing an old plan after a mutation. Cache hits do not take the lock or query Elasticsearch.
- Invalidate on rule-management changes and lifecycle cleanup; the evaluator's `LastFiredUtc` bookkeeping suppresses repository notifications so deliveries do not churn the hot-path plan.
